---
title: "Working with ODK Central: connect, monitor, and pull form data (for data analysts)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working with ODK Central: connect, monitor, and pull form data (for data analysts)}
...
<div class="mermaid">
flowchart TD
    A["Field teams collect on phones"] --> B["ODK Central server"]
...
2. **The package:**
   ...
   ```
3. **ODK Central credentials.** Store them once in your `.Renviron` so they never end up in your
   scripts. Open it with `usethis::edit_r_environ()`, add three lines, then **save and restart R**:
   ```
...
In the ODK Central web interface: open (or create) a **`test`** project ▸ **New** ▸ **Form** ▸ upload
that `.xlsx` ▸ **Publish**. Then submit **2–3 fake entries**: on the form page click **Submit** (the
Enketo web form), fill in made-up values, and send. Now there is data to work with.
...
`init_odk_connection()` reads `ODK_URL` / `ODK_USER` / `ODK_PASS` from your `.Renviron`, signs in, and
returns a session you pass to the other functions as `con =`.

### Find your project and form

You refer to forms by a **project id** (a number) and a **form id** (a string). Discover them:
...
`eri_odk_sync()` downloads every submission and writes it as a typed Parquet file into the **raw**
layer, then stamps the registry with the sync time:
...
From here the ODK data flows through the **exact same pipeline** as a surveillance extract. Quality-
check it against a schema with `run_dq_checks()` (the **[surveillance ingest
guide](da-ingest-guide.html)** walks through authoring a schema and reading the flags), write the
cleaned result to `staged/`, then **approve** it into the canonical `processed/` layer:
...
A second bundled XLSForm has a repeated `larva_sample` group (blackfly species + larva count) under
each river-prospection site:
...
Pass `tables = TRUE` to `download_odk_form()` to get **all** of the form's tables back as a named list
...
The **child** table holds one row per larva sample. Its `PARENT_KEY` points back to the parent's
`KEY`:
...
Each table then flows through the same quality-check → stage → **approve** gate as any other extract.
Approve the parent and its repeats together so they stay in step.
...
> **One thing that intentionally stays:** `eri_odk_deregister()` *soft*-deletes — it flips the
> registry entry to `active: false` rather than erasing it, so the record of what was once tracked
> (and its sync history) survives for audit. That inactive entry is harmless; it will not show up in
...
You have run the full ODK loop: connect, monitor, manage collectors, sync to the governed pipeline,
...
- For the full data-quality and approval mechanics, see the
  [surveillance ingest guide](da-ingest-guide.html); for the whole menu of functions, the
  [reference](../reference/index.html). The
  [guide index](https://github.com/thecartercenter/erifunctions/blob/main/docs/guides.md) tracks what

...
A non-empty tibble tells you exactly which cell to fix (e.g. `river_stage` value `"purple"` →
*"value(s) not in the choice list"*). Once it is clean, drop `dry_run` to send them:
...
> *hard*-deletes the entry (active or already-inactive) so the shared registry is left exactly as we
> found it.

---
title: "Catching anomalies in a new surveillance extract (for epidemiologists)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Catching anomalies in a new surveillance extract (for epidemiologists)}
...
<div class="mermaid">
flowchart TD
    A["New extract"] --> B["run_dq_checks(): cell-level QC"]
    B --> C["Aggregate to weekly counts"]
    C --> D["add_anomaly_pct_change(): spikes"]
    C --> E["add_anomaly_gaps(): missing weeks"]
    B --> F["add_anomaly_consistency(): cross-field rules"]
    B --> G["add_anomaly_spatial(): admin names"]
    D --> H["Investigate before analysing"]
    E --> H
    F --> H
    G --> H
</div>
...
`remotes::install_github("thecartercenter/erifunctions")`. Everything here is offline.
...
reference boundaries from Azure; our bundled case schema has neither, so it skips cleanly rather than
erroring:
...
Resolve those, and the curve, incidence, and maps you build next rest on numbers you've actually
interrogated.

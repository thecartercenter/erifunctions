...
you are not acting as yourself, see the automation callout below. Pass `quiet = TRUE` to suppress
the message in code that connects in a loop.
...
> from your token; a service principal token has no user claim, so that verification quietly falls
> back to whatever `ERI_ANALYST_ID` says, and your approval log stops agreeing with Azure's own
> record of who wrote the file.

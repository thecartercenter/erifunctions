...
flowchart TD
    ...
    E --> F["eri_approve(): staged -> processed/ + catalog"]
</div>
```
...
## 4. Approve it {#approve}

Once the numbers look right, promote the staged report to the canonical `processed/` layer with the
same human gate every dataset passes through. CMR uses `disease = "rblf"` and `data_type = "cmr"`:

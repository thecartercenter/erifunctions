...
flowchart TD
    ...
    F --> G["eri_cmr_dq_report(): one combined flags tibble across every measure"]
    G --> H["eri_dq_flag_resolve() issue by issue, then eri_logs_resolve() each measure"]
    H --> I["eri_approve_cmr(): approve everything at once, only if all clean"]
</div>
```
...
If the dry run comes back with nothing skipped and no warnings, you'll see a plain "Dry run clean --
ready to run for real" instead of having to infer it from an absence of complaints. If it does flag
a skipped sheet, that's logged so you have a stable reference to attach a note to later, once you've
looked into it and fixed whatever needed fixing:

A **duplicate field code** (a real template defect -- the same machine-readable code typed twice in
a sheet's row 5, most often seen on "RB Ento Surveys") is different: it's not a survivable warning.
It **aborts the dry run entirely, for the whole workbook** -- no plan is produced, not even for
sheets unrelated to the defective one (see [ADR-0022](https://github.com/thecartercenter/erifunctions/blob/main/docs/adr/0022-cmr-duplicate-field-code-blocks-workbook.md)).
The failure is still logged (`status = "error"`), so it's visible via `eri_logs()`, but there's no
plan to review around it -- flag it to whoever maintains the CMR template and get a corrected file
before retrying.

```{r split-dry-log, eval=FALSE}
...
Then stage for real, one parquet per sheet, in its disease/measure folder. If this country's raw file
still also needs to reach the legacy contractor pipeline during the Phase-3 parallel run, `mirror_pipeline`
uploads it there in the same call -- one step instead of a separate manual upload:
...
Keep `plan` around for the next two steps. Lost it, or ran the split in an earlier session? Recover it
without rerunning anything (the routing table is persisted in the op-log every real run writes):

```{r recover-plan, eval=FALSE}
plan <- eri_cmr_last_plan("uga", "202406")
```

**Re-splitting a corrected file.** Fixed something upstream and re-splitting a `_fixed.xlsx` copy for
a period you already split? By default `eri_split_cmr()` only *warns* about the now-superseded prior
staged file(s) -- it never deletes anything without you opting in, since this is the one place in the
package that can remove previously-staged data:

```{r split-supersede, eval=FALSE}
...
Leave it `FALSE` (the default) if you'd rather review the warning and remove the old file by hand.

## 5. Check data quality before approving {#dq-check}

> **Prefer to do this interactively?** Sections 5 and 6 below are the scriptable core --
> [`eri_dq_review()`](../reference/eri_dq_review.html) walks you through the same check → fix →
> re-check → approve loop as one guided, menu-driven session (fix a value in the workbook, adjust
> the schema, mark a flag not important/noted, force-approve with a typed confirmation, and submit
> a schema fix for a maintainer to fold in -- all from one prompt): `eri_dq_review("uga", "202406")`.
...
measure `plan` routed, in one call, and gives you back **one combined tibble** -- every flag from
every measure -- instead of twelve separate `dq_report()` printouts:
...
`eri_dq_flag_resolve()` updates the *logged* flag, not this run's in-memory `flags` tibble --
update your own copy the same way [`eri_dq_review()`](../reference/eri_dq_review.html) tracks
triage between loop iterations, so anything you print or export below reflects the decision:
...
Once every flag in a measure's report has been triaged, close out that measure's whole log entry --
this is what actually unblocks [`eri_approve_cmr()`](#approve) below. Skip `note` and it auto-summarizes
from the per-flag decisions you just made instead of leaving the entry's own note blank:
...
If you'd rather scan the *logged* state across the team's whole backlog (not just this run's
in-memory tibble), `eri_logs()` still works the same way, one row per measure:
...
Handing this back to a country or pasting it into a Teams thread? [`eri_dq_export()`](../reference/eri_dq_export.html)
...
One CMR workbook fans out into several diseases/measures; [`eri_approve_cmr()`](../reference/eri_approve_cmr.html)
approves all of them in one call **but only if every one is clean** -- reviewed via the DQ-check step
above, with no outstanding flags:
...
If anything's still outstanding (a measure was never DQ-checked, or has unresolved flags), **nothing
is approved** -- you get a task list back instead:

```{r approve-blocked, eval=FALSE}
eri_approve_cmr("uga", "202406")
...
Each disease's data is now canonical and discoverable in the catalog on its own coordinates. (You can
still approve one measure at a time with plain [`eri_approve()`](../reference/eri_approve.html) --
`eri_approve_cmr()` is a convenience wrapper around exactly that, looped, with the DQ gate in front.)

`eri_approve_cmr()` also writes its own log entry recording exactly which `dq_flags` log(s) it
verified clean for this approval (`dq_reviewed`) -- so the audit trail from "this data is now
...
That's the monthly loop: **upload → stage → split → DQ-check → approve.**

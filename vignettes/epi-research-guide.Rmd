...
Uploading put the data in Azure; now pull a copy *into your project* to work on. Pulling (rather
than just reading a local file) records **where the data came from and when** in
`research.yaml`, so your inputs are always traceable:

```{r artifact-pull}
eri_artifact_pull("mtcars_study_data", dest = "data")
#> ✔ Pulled "mtcars_study_data" to data/cars.csv.
```
...
#> ✔ Snapshot "before-break" → research/mtcars_study/snapshots/20260617T150500Z/ (1 file).
...
#>   kind  source                                          pulled_at            updates archived
#>   <chr> <chr>                                           <chr>                  <int> <lgl>
#> 1 pull  artifacts/study_data/mtcars_study_data/cars.csv 2026-06-17T15:30:00Z       1 TRUE
```
...
surveillance dataset, onboarding a new country, and more). See [the guide index](../docs/guides.md)

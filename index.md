...
The full set (**Sandbox** = runs safely with no real country ever
touched — on the built-in `atlantis` training country, fully offline, or
with public/placeholder data under a non-real country code):
...
| [Triaging DQ flags interactively](https://thecartercenter.github.io/erifunctions/articles/da-dq-review-guide.html) | Data analysts working a CMR’s DQ flags through one guided menu ([`eri_dq_review()`](https://thecartercenter.github.io/erifunctions/reference/eri_dq_review.md)) instead of the underlying functions, ending in a handback file ([`eri_dq_export()`](https://thecartercenter.github.io/erifunctions/reference/eri_dq_export.md)) | ~20 min | Azure | Yes |
...
| [Answering ad-hoc requests with SQL](https://thecartercenter.github.io/erifunctions/articles/da-adhoc-guide.html) | Data analysts running SQL across approved datasets (roll-ups, joins) with the [`eri_query()`](https://thecartercenter.github.io/erifunctions/reference/eri_query.md) DuckDB layer | ~15 min | Azure | No |
...
Two templates ship with the package. Pull either one with
...
`country` / `disease` / `data_source` / `data_type`; the legacy
four-axis path form and schema names are being migrated under \#175.)

### Data catalog

Every file promoted by
[`eri_approve()`](https://thecartercenter.github.io/erifunctions/reference/eri_approve.md)
is automatically registered in `_catalog/data_catalog.yaml`. Query it to
see what data exists on the system:
...
|----|----|
| [`get_azure_storage_connection()`](https://thecartercenter.github.io/erifunctions/reference/get_azure_storage_connection.md) | Authenticate with Azure (browser or service principal) |
| [`init_odk_connection()`](https://thecartercenter.github.io/erifunctions/reference/init_odk_connection.md) | Authenticate with ODK Central |
...
| [`eri_data_model()`](https://thecartercenter.github.io/erifunctions/reference/eri_data_model.md) | Show the known `data_source` / `data_type` / `format` values |
...
| `eri_dq_export(flags, file, format)` | Render a DQ flags tibble (CMR or plain [`run_dq_checks()`](https://thecartercenter.github.io/erifunctions/reference/run_dq_checks.md)) to a self-contained HTML/Markdown handback file |
...
| [`eri_odk_list_registered()`](https://thecartercenter.github.io/erifunctions/reference/eri_odk_list_registered.md) | List all active registered forms |
...
| [`list_odk_projects()`](https://thecartercenter.github.io/erifunctions/reference/list_odk_projects.md) | List all ODK projects |
...
| Function | What it does |
|----|----|
| [`eri_template_list()`](https://thecartercenter.github.io/erifunctions/reference/eri_template_list.md) | List bundled and Azure-hosted templates |

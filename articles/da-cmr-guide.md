...
`sheet` (the real sheet name) and `excel_row` (each row’s actual
position in the workbook) are always there alongside `country` and the
field-code columns – `sheet` is what lets a schema covering several
sheets routed to the same measure (e.g. the combined training schemas)
tell which one a row came from.
...
infer it from an absence of complaints. If it does flag a skipped sheet,
that’s logged so you have a stable reference to attach a note to later,
once you’ve looked into it and fixed whatever needed fixing:

A **duplicate field code** (a real template defect – the same
machine-readable code typed twice in a sheet’s row 5, most often seen on
“RB Ento Surveys”) is different: it’s not a survivable warning. It
**aborts the dry run entirely, for the whole workbook** – no plan is
produced, not even for sheets unrelated to the defective one (see
[ADR-0022](https://github.com/thecartercenter/erifunctions/blob/main/docs/adr/0022-cmr-duplicate-field-code-blocks-workbook.md)).
The failure is still logged (`status = "error"`), so it’s visible via
[`eri_logs()`](https://thecartercenter.github.io/erifunctions/reference/eri_logs.md),
but there’s no plan to review around it – flag it to whoever maintains
the CMR template and get a corrected file before retrying.
...
If the country’s CMR routing schema declares `cross_consistency:` rules
(Ethiopia, as of this writing), a flag comparing data *across* sheets
can also show up here, with `sheet = "(cross-sheet)"`, a blank
`excel_row`, and `cross = TRUE` – see the [DQ review
guide](https://thecartercenter.github.io/erifunctions/articles/da-dq-review-guide.html#split)
for what that means and how to work through one.

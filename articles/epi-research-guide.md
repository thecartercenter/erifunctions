...
**What matters here is the data management and the research lifecycle**,
not the model.
...
flowchart LR A\["Start a project  
(scaffold + GitHub)"\] --\> B\["Add data  
+ metadata"\] B --\> C\["Source data  
into the project"\] C --\> D\["Analyse  
(your code)"\] D --\> E\["Save outputs  
+ figures"\] E --\> F\["Tag a version  
(citable freeze)"\] F -.-\>\|weeks later\| G\["Resume  
+ check status"\] G --\> H\["New data arrives  
(re-upload + re-pull)"\] H --\> D F --\> Z\["Clean up"\]
...
4.  **A GitHub account**, and [GitHub
    Desktop](https://desktop.github.com/) if you prefer clicking to
    typing. We will use it below.

One optional comfort setting: `erifunctions` narrates what it is doing.
If you find it chatty, turn it down for the session (or permanently in
your `.Rprofile`):
...
Now open the project in RStudio: **File ▸ Open Project**, navigate into
`mtcars_study/`, and open it. Keeping RStudio rooted at the project
(rather than opening loose files) matters for the next step.
...
> **Prefer clicking? Use GitHub Desktop.** Open GitHub Desktop ▸ **File
> ▸ Add Local Repository**, choose your `mtcars_study/` folder, and
> click **“create a repository”** when it offers. Then **Publish
> repository** to put it on GitHub. From then on you **commit** and
> **push** with the buttons in GitHub Desktop instead of typing git
> commands. Everywhere below where we say “commit”, that is the GitHub
> Desktop **Commit to main** button.

### Pin your software versions (this turns the check on)

`renv` records the exact version of every package the study uses, so the
analysis runs the same way next year as it does today:
...
## 4. Source the data into your project

Uploading put the data in Azure; now pull a copy *into your project* to
work on. We use
[`eri_research_pull()`](https://thecartercenter.github.io/erifunctions/reference/eri_research_pull.md)
and hand it the artifact’s path. Pulling this way (rather than just
reading a local file) records **where the data came from and when** in
`research.yaml`, and sets up the **reversible updates** you’ll rely on
in §11 when fresh data arrives:
...
> **Two ways to pull, on purpose.**
> `eri_artifact_pull("mtcars_study_data")` is the quick registry
...
```
    ...
    #> 
    #> Call:
    #> lm(formula = mpg ~ wt + hp + disp, data = cars)
    #> 
    #> Residuals:
    #>     Min      1Q  Median      3Q     Max 
    #> -2.9515 -1.4818 -0.1307  1.4072  3.3112 
    #> 
    #> Coefficients:
    #>               Estimate Std. Error t value Pr(>|t|)    
    #> (Intercept) 30.6052577  2.6986421  11.341 9.32e-09 ***
    #> wt          -1.8050509  1.0698184  -1.687   0.1122    
    #> hp          -0.0401358  0.0160570  -2.500   0.0245 *  
    #> disp        -0.0007664  0.0100435  -0.076   0.9402    
    #> ---
    #> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
    #> 
    #> Residual standard error: 2.024 on 15 degrees of freedom
    #> Multiple R-squared:  0.7677, Adjusted R-squared:  0.7212 
...
Push your results back to Azure so they are safe and tied to the
project. Save the fitted model object, and a figure:
...
## 7. Look at the state of your project

At any point, ask the project where it stands.
[`eri_research_status()`](https://thecartercenter.github.io/erifunctions/reference/eri_research_status.md)
prints what data you have pulled, how many times each input has been
updated, and how many outputs, snapshots, and tags exist:
...
Then **commit** your code (the `usethis::use_git()` first commit, or the
**Commit to main** button in GitHub Desktop) and close your laptop.
Nothing is lost.

### Resuming, weeks later

Reopen the **project** in RStudio (open the project, not loose files, so
`renv` re-activates your pinned environment automatically). Then ask the
project to reintroduce itself:
...
We have just finished our first real result, so tag it. Make sure your
analysis code is **committed first** (the tag records the commit, and
warns you if the working tree has uncommitted changes):
...
Now pull the new version into the project. Because you have pulled this
input before, `erifunctions` **archives the previous copy**, swaps in
the new one, and records that this input has now been updated:
...
If you ran this walkthrough for practice, remove everything it created
so you leave no trace in Azure. This deletes the project folder, all
snapshots and tags, and the artifact:
...
> protected data safe. When in doubt, ask before you delete.

## What’s next

You have now done the full research lifecycle: start, version-control,

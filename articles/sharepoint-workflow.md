...
opens a browser window for interactive authentication the first time.
The token is cached by `AzureAuth` so subsequent calls within a session
do not re-prompt.
...
downloads the file to a temp location and reads it based on the
extension (`.xlsx`, `.csv`, `.parquet`, `.rds`).
...
For an unknown extension the function returns the local temp path so you
can read it manually:
...
Use `overwrite = FALSE` to prevent accidentally replacing a file:
...
The most common pattern is: pull data from SharePoint, run DQ checks,
generate a report, push the report back.
...
- Tokens are cached in `~/.AzureR/` by `AzureAuth`. If you are prompted
  every session, check that `~/.AzureR/` is writable.
- For scripted / CI use, prefer service-principal auth configured
  through `Microsoft365R::create_sharepoint_site()` with
  `auth_type = "client_credentials"`.
- All `eri_sharepoint_*` functions guard against `Microsoft365R` not
  being installed and give a clear install instruction if it is absent.

...
test_that("eri_schema_validate accepts range_when's `in`/`not_in` ops (issue #335 fix)", {
  # .ERI_DQ_RANGE_OPS (R/dq.R), which the actual engine validates against, has
  # long included `in`/`not_in` -- this validator's own op check had drifted
  # to a stale hardcoded list that didn't, so a legitimately valid range_when
  # using `in` would have been wrongly flagged. Fixed alongside adding the
  # parallel allowed_values_when validation block below.
  schema <- list(
    country  = "Uganda",
    disease  = "oncho",
    temporal = list(year_col = "Year", period_col = "EpiWeek"),
    columns  = list(
      Year        = list(required = TRUE, type = "numeric"),
      EpiWeek     = list(required = TRUE, type = "numeric"),
      status      = list(required = FALSE, type = "character"),
      target_pop  = list(required = FALSE, type = "numeric", range = list(1, 100),
                         range_when = list(column = "status", op = "in", value = list("ongoing")))
    )
  )
  tmp <- tempfile(fileext = ".yaml")
  yaml::write_yaml(schema, tmp)
...
test_that("eri_schema_validate flags an allowed_values_when referencing an unknown column", {
  schema <- list(
    country  = "Uganda",
    disease  = "oncho",
    temporal = list(year_col = "Year", period_col = "EpiWeek"),
    columns  = list(
      Year            = list(required = TRUE, type = "numeric"),
      EpiWeek         = list(required = TRUE, type = "numeric"),
      treatment_round = list(required = FALSE, type = "numeric", allowed_values = list(1, 2, 4),
                             allowed_values_when = list(column = "NoSuchCol", op = ">", value = 0))
    )
  )
  tmp <- tempfile(fileext = ".yaml")
  yaml::write_yaml(schema, tmp)
  result <- suppressWarnings(eri_schema_validate(tmp))
  expect_true(any(result$field == "columns.treatment_round.allowed_values_when.column" &
                    result$issue_type == "unknown_column_reference"))
  unlink(tmp)
})

test_that("eri_schema_validate flags an allowed_values_when with an invalid op", {
  schema <- list(
    country  = "Uganda",
    disease  = "oncho",
    temporal = list(year_col = "Year", period_col = "EpiWeek"),
    columns  = list(
      Year            = list(required = TRUE, type = "numeric"),
      EpiWeek         = list(required = TRUE, type = "numeric"),
      target_pop      = list(required = FALSE, type = "numeric"),
      treatment_round = list(required = FALSE, type = "numeric", allowed_values = list(1, 2, 4),
  ...
  yaml::write_yaml(schema, tmp)
  result <- suppressWarnings(eri_schema_validate(tmp))
  expect_true(any(result$field == "columns.treatment_round.allowed_values_when.op" &
                    result$issue_type == "invalid_value"))
  unlink(tmp)
})

test_that("eri_schema_validate is clean on a valid allowed_values_when", {
  schema <- list(
    country  = "Uganda",
    disease  = "oncho",
    temporal = list(year_col = "Year", period_col = "EpiWeek"),
...
new_schemas <- list(
  ...
  c("ssd",    "lf",      "programmatic", "treatment"),
  c("sdn",    "lf",      "programmatic", "treatment"),
  c("ssd",    "lf",      "programmatic", "mmdp"),
  c("sdn",    "lf",      "programmatic", "mmdp"),
  c("ssd",    "lf",      "programmatic", "tas"),
  c("sdn",    "lf",      "programmatic", "tas"),
  c("ssd",    "oncho",   "programmatic", "prevalence"),
  c("sdn",    "oncho",   "programmatic", "prevalence"),
  c("ssd",    "oncho",   "programmatic", "entomology"),
  c("sdn",    "oncho",   "programmatic", "entomology"),
  c("ssd",    "rblf",    "programmatic", "training"),
  c("sdn",    "rblf",    "programmatic", "training"),

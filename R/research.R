...
eri_research_scaffold <- function(name, country, disease, description,
  ...
  writeLines(c(
    ...
    "        # Keep this list in sync with erifunctions' DESCRIPTION Imports system requirements.",
    "        name: System libraries (sf -> gdal/proj/geos/udunits; AzureStor -> curl/openssl)",
    "        run: |",
    "          sudo apt-get update",
    "          sudo apt-get install -y libcurl4-openssl-dev libssl-dev libgdal-dev libproj-dev libgeos-dev libudunits2-dev",
    "      - if: steps.lock.outputs.present == 'true'",
    "        uses: r-lib/actions/setup-r@v2",
    "        with:",
    "          # CRAN packages as Linux binaries (no source build)",
